...
export default function QuestionBankPage() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      let res;
      if (searchQuery.trim()) {
        res = await questionBankService.search(searchQuery.trim(), {
          category,
          difficulty: difficulty || undefined,
        });
      } else {
        res = await questionBankService.getQuestions(category, filters);
      }
  ...
  const handleCreate = async () => {
    try {
      if (editingQuestion) {
        await questionBankService.update(editingQuestion.id, formData);
